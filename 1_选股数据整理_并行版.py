...
# ===读取准备数据
# 读取所有股票代码的列表
stock_code_list = get_stock_code_list_in_one_dir('D:/data/数据/stock-trading-data-pro/')
print('股票数量：', len(stock_code_list))

# 导入上证指数，保证指数数据和股票数据在同一天结束，不然会出现问题。
index_data = import_index_data('D:/data/数据/index/sh000300.csv', back_trader_end=date_end)
...
def calculate_by_stock(code):
    ...
    print(code, '开始计算')
    # =读入股票数据
    path = 'D:/data/数据/stock-trading-data-pro/%s.csv' % code
...
if __name__ == '__main__':
    ...
    # 试运行
    # df = calculate_by_stock('sh600000')
    # exit()
